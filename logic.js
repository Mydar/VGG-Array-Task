...
for (i = 0, j = 1; i < color.length, j < 8; i++, j++) {
    if (i == 0) {
...
for (i = 0; i < employees.length; i++) {
  if (employees[i].Job == 'Web Developer' && employees[i].Country == 'Nigeria') {
    nigerianWebDevs.push(employees[i]);
  }console.log(nigerianWebDevs);
}
...
let salaryValues = [];
for (i = 0; i < employees.length; i++) {
    salaryValues.push(employees[i].salary);
  }
...
function findMax(arr) {
 ...
 console.log(findMax(salaryValues));
